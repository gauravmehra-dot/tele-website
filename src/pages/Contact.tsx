...
export default function Contact() {
  ...
  const set = (f: keyof ContactForm, v: string) => {
    setForm(p => ({ ...p, [f]: v }))
  ...
  const handleSubmit = () => {
    ...
    else if (form.message.trim().length < 10) e.message = 'Please provide a little more detail'

    setErrors(e)
    if (Object.keys(e).length === 0) setSubmitted(true)
  }

  const baseInputClass = 'w-full px-4 py-3 rounded-xl border bg-[#F5F9FF] text-[#1A2B3C] text-sm focus:outline-none focus:ring-2 placeholder-[#94A3B8] transition-all'
  const inputClass = `${baseInputClass} border-[#E2EBF6] focus:border-[#0A6EBD] focus:ring-[#0A6EBD]/20`
  const errorInputClass = `${baseInputClass} border-red-400 focus:border-red-500 focus:ring-red-200`
  const labelClass = 'block text-sm font-semibold text-[#1A2B3C] mb-1.5'

  const fieldClass = (f: keyof ContactForm) => (errors[f] ? errorInputClass : inputClass)
  const fieldError = (f: keyof ContactForm) =>
    errors[f] ? <p className="mt-1.5 text-xs text-red-600">{errors[f]}</p> : null
                ...
                <div className="bg-white rounded-3xl border border-[#E2EBF6] p-8 shadow-sm">
                  ...
                  <div className="space-y-5">
                    <div>
                      ...
                      <input className={fieldClass('name')} placeholder="Emma Robertson" value={form.name} onChange={e => set('name', e.target.value)} />
                      {fieldError('name')}
                    </div>
                    <div className="grid sm:grid-cols-2 gap-5">
                      <div>
                        <label className={labelClass}>Email Address *</label>
                        <input type="email" className={fieldClass('email')} placeholder="emma@example.com.au" value={form.email} onChange={e => set('email', e.target.value)} />
                        {fieldError('email')}
                      </div>
                      <div>
